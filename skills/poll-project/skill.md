...
Local counterpart to the OpenClaw `github-poller`. One invocation = one pass over the project's Ready issues, dispatching a `project-task` subagent for each one (up to `maxConcurrent`).

Designed to be triggered from Claude Code on the operator's laptop, optionally wrapped in `/loop 5m /poll-project` for a periodic re-poll within a long-running session.
...
The poller needs the same IDs the `project-task` skill consumes. Fetch them via GraphQL using local `gh` auth:
...
Move the card to In Progress with the same mutation the `project-task` skill uses:
...
Use Claude Code's `Agent` tool with `subagent_type: "general-purpose"`. Pass the full contents of `skills/project-task/skill.md` plus the env-equivalent values inline in the prompt. The subagent runs in-process — no ACP, no SSM.
...
--- BEGIN project-task skill ---
<full contents of skills/project-task/skill.md>
--- END project-task skill ---
